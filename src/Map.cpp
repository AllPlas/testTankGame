...
Map::Map() {
  ...
  for (std::size_t i{}; i < 2; ++i) {
    m_images[i].loadFromFile(data::assetsDir / "Map/" / std::to_string(i).append(".png"));
...
void Map::DrawMap(sf::RenderWindow& window) {
  for (std::size_t i{}; i < m_map.size(); ++i)
    for (std::size_t j{}; j < m_map[i].size(); ++j) {
      ...
      if (m_map[i][j] == '#')
        m_sprite.setTexture(m_textures[static_cast<std::size_t>(MapObject::barrier)]);
